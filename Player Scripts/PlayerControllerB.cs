...
public class PlayerControllerB : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Define the target position, only changing the X  axis
        targetPosition = new Vector3(mouseWorldPosition.x, transform.position.y, transform.position.z);
    }

    void FixedUpdate()
    {
        // Calculate the next position using MoveTowards
        Vector3 newPosition = Vector3.MoveTowards(targetPosition, transform.position, moveSpeed * Time.deltaTime);
